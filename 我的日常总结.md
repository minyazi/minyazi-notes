...
6、commit;
```
测试结果：在session_1提交之前，该条SQL会锁等待超时，只有session_1提交之后，该条SQL才能执行成功。  
间隙锁解决办法：更新（删除）数据时，少用范围检索；尽量不去更新（删除）不存在的记录，存在才更新（删除）。  
PS：  
如何查看数据库中的锁等待情况？
```SQL
...
`[2016/12/21]`  
1、常用命令  
git config --global user.name “Your Name”   ##配置用户名字  
git config --global user.email email@example.com   ##配置用户邮箱  
git init   ##初始化本地仓库  
git status [文件名/目录名]   ##查看仓库状态  
git diff [文件名/目录名]   ##查看修改内容  
git add 文件名/目录名   ##把文件或目录修改添加到暂存区  
git checkout -- 文件名/目录名   ##撤销工作区的修改  
git reset HEAD文件名/目录名   ##把暂存区的修改回退到工作区  
git commit -m “提交说明”   ##把文件或目录修改提交到仓库  
git push   ##同步到GitHub  
git log [--pretty=oneline]   ##查看提交历史  
git reflog   ##查看命令历史  
git reset --hard 版本号   ##修改仓库版本。版本号即为commit id，当前版本可用HEAD表示，上一个版本可用HEAD^表示，上上一个版本可用HEAD^^表示，往上100个版本可用HEAD~100表示。  
git pull   ##从GitHub同步到本地仓库
## Java
1、常用接口  
java.lang.Cloneable  
...
【并发与并行的区别】并发和并行是即相似又有区别的两个概念，并行是指两个或者多个事件在同一时刻发生；而并发是指两个或多个事件在同一时间间隔内发生。在多道程序环境下，并发性是指在一段时间内宏观上有多个程序在同时运行，但在单处理机系统中，每一时刻却仅能有一道程序执行，故微观上这些程序只能是分时地交替执行。倘若在计算机系统中有多个处理机，则这些可以并发执行的程序便可被分配到多个处理机上，实现并行执行，即利用每个处理机来处理一个可并发执行的程序，这样，多个程序便可以同时执行。  
